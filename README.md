...
**Page furniture** — set in Appostta settings and printed on every certificate: a **page border** (ornamental, double rule or single rule), a **background watermark** (a phrase repeated faintly behind the content, empty for a plain page), a **certification mark** (a line of italic text that can sit after any row or after the last one), and the **verification line**, which is always the last line on the page and whose wording is yours.

**Layout** — the heading, the rows, the mark and the reference strip are cells of one ruled table on a portrait sheet. Each row prints its heading above its value; ticking **value on the same line** puts them side by side instead, which suits short rows like a place or a date. That choice is frozen onto a record with the rest of the row.

**Values that fill themselves in** — a row value, the certification mark and both notes accept `{{number}}`, `{{date}}`, `{{day}}`, `{{month}}`, `{{year}}` and `{{url}}`. They are substituted when the certificate is drawn, not when the value is saved, so a row carrying the number or the issue date follows the record if either is edited. An unrecognised name is left on the page as typed, so the mistake is visible rather than silent.
